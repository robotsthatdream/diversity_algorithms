...
from diversity_algorithms.analysis.population_analysis import get_coverage
...
params_commandline = {
    ...
    "outfile_pattern": RunParam("","coverages_evolvability_gen%d.npz", "Output file pattern"),
...
def coverage_from_file(file,  min_x, max_x, nb_bin, verbose=False):
	if(verbose):
		print("Computing coverage for '%s'..." % file, end='')
	points=du.get_points_from_genfile(file)
	cov = get_coverage(min_x,max_x,nb_bin,points)
	if (verbose):
...
if(__name__=='__main__'):
	...
	coverages = list(futures.map(lambda f: coverage_from_file(f, min_x, max_x, nb_bin, verbose), evofiles))
	print("Done ! Saving data...")
	outfile = os.path.join(rundir,outfile_pattern % gen) 
	np.savez(outfile, coverages=coverages)
	print("Evolvability data saved to %s" % outfile)
